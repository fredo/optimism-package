...
def get_config(
    ...
    chain_path = launcher.network if launcher.network in ethereum_package_constants.PUBLIC_NETWORKS else ethereum_package_constants.GENESIS_CONFIG_MOUNT_PATH_ON_CONTAINER + "/genesis-{0}.json".format(launcher.network_id)
    cmd = [
        "node",
        "--datadir=" + EXECUTION_DATA_DIRPATH_ON_CLIENT_CONTAINER,
        "--chain={0}".format(chain_path),
        "--http",
    ...
    if persistent:
        ...
        files["/data/rpc_store_database"] = Directory(
            persistent_key="rpc-store-{0}".format(service_name),
            size=1024,  # 1GB default size
        )
    ...
    if observability_helper.enabled:
        cmd.append("--metrics={0}:{1}".format("0.0.0.0", observability.METRICS_PORT_NUM))
